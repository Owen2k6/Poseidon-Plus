...
public class World implements IBlockAccess {
    ...
    public final List<TileEntity> c = new ArrayList<>();
    private final List<TileEntity> G = new ArrayList<>();
    public final List<EntityHuman> players = new ArrayList<>();
    public final List<Entity> e = new ArrayList<>();
    ...
    private final List<TileEntity> tileEntitiesToUnload;
    ...
    public void cleanUp() {
        ...
        if (!this.G.isEmpty()) {
            ...
            for (TileEntity tileentity1 : this.G)
            {
                if (!tileentity1.g())
                {
    ...
    public boolean containsEntity(AxisAlignedBB axisalignedbb) {
        List<Entity> list = this.b(null, axisalignedbb);
    ...
    protected void j() {
        ...
        for (EntityHuman player : this.players)
        {
            ...
            i = MathHelper.floor(player.locX / 16.0D);
            j = MathHelper.floor(player.locZ / 16.0D);
            byte b0 = 9;
        ...
        for (ChunkCoordIntPair chunkcoordintpair : this.P)
        {
    ...
    public boolean a(boolean flag) {
        ...
        if (i != this.F.size()) {
            ...
            for (int j = 0; j < i; ++j) {
                NextTickListEntry nextticklistentry = this.E.first();
    ...
    public List<Entity> b(Entity entity, AxisAlignedBB axisalignedbb) {
        this.R.clear();
    ...
    public void b(int i, int j, int k, TileEntity tileentity) {
        ...
        for (IWorldAccess iWorldAccess : this.u)
        {
            iWorldAccess.a(i, j, k, tileentity);
        }
    }

    public int a(Class<? extends Entity> oclass) {
        int i = 0;

        for (Entity value : this.entityList)
    ...
    public void a(EntityHuman entityhuman, int i, int j, int k, int l, int i1) {
        for (IWorldAccess iWorldAccess : this.u)
        {
            iWorldAccess.a(entityhuman, i, j, k, l, i1);
        }
    }
